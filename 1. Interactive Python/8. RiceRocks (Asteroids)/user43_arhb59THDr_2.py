...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            explosion_center = [self.image_center[0] + self.age * self.image_size[0],
                          self.image_center[1]]
    ...
    def collide(self, other_object):
        ...
        s_rad = self.get_radius()
        o_pos = other_object.get_position()
        o_rad = other_object.get_radius()
        return dist(s_pos, o_pos) < s_rad + o_rad
...
def group_collide(group, other_object):
    ...
    for sprite in set(group):
        if sprite.collide(other_object):
            ...
            explosion = Sprite(sprite.get_position(),
                              sprite.vel, sprite.angle,
                              sprite.angle_vel, explosion_image,
...
def rock_spawner():
    ...
    if started and len(rock_group) < 12:
        ...
        rock_vel = [(random.random() * .6 - .3) * score // 10 + .01,
                    (random.random() * .6 - .3) * score // 10 + .01]
